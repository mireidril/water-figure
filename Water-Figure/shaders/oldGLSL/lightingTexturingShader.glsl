...
void main()
{
    ...
    if (filledData[2])
    {
        diffuseColor=vec4(texture2D(textureUnitDiffuse, uvs).rgb, 1.0);
        specularColor=vec4(texture2D(textureUnitSpecular, uvs).rgb, 1.0);
    }
    
    // If no normal
    if (!filledData[1]) gl_FragColor = diffuseColor;
    else
    {
